...
export const useHistoryDashboard = (userEmail: string) => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      // Transform the data to match our interface with FIXED mapping
      const transformedItems: ReadmeHistoryItem[] = items.map((item: any) => {
        ...
        return {
          ...
          confidence: typeof item.confidence === 'object' ? 
      ...
      console.log('✅ FIXED DASHBOARD - Successfully loaded history items:', transformedItems.length);
  ...
  const deleteHistoryItem = async (requestId: string) => {
    try {
      ...
      setHistoryItems(prev => prev.filter(item => item.requestId !== requestId));
