...
export const useReadmeGeneratorSecure = () => {
  ...
  const pollForCompletion = useCallback(async (arn: string) => {
    let attempts = 0;
    ...
    const poll = async () => {
      try {
        ...
        if (statusData.status === 'SUCCEEDED') {
          let output;
          try {
            output = statusData.output ? JSON.parse(statusData.output) : statusData;
          } catch (parseError) {
            console.log('🔧 Output is not JSON, using as-is:', statusData.output);
            output = statusData;
          }
          ...
          const errorMessage = statusData.error || 'Step Functions execution failed';
          setError(errorMessage);
          setLoading(false);
          toast.error('Generation failed', {
            description: errorMessage,
          });
        } else if ((statusData.status === 'RUNNING' || statusData.status === 'EXECUTING') && attempts < maxAttempts) {
        ...
        if (attempts < 3) {
          console.log(`🔧 Retrying after error (attempt ${attempts}/3):`, err.message);
  ...
  const generateREADME = useCallback(async (githubUrl: string) => {
    ...
    try {
      ...
      if (data.executionArn) {
        ...
        console.log('🔧 Starting polling for execution ARN:', data.executionArn);
        ...
        throw new Error('No execution ARN returned from API - generation may not have started properly');
      }
