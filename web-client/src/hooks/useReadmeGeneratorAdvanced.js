...
const initializeSFNClient = async () => {
  if (!sfnClient) {
    await initializeAWS();
    throw new Error('Direct AWS SDK calls should be replaced with API Gateway calls for security');
  }
  return sfnClient;
};

// Phase 3 Complete Workflow ARN with DynamoDB + Email
const PHASE3_WORKFLOW_ARN = 'arn:aws:states:us-east-1:695221387268:stateMachine:complete-readme-generator-workflow';
