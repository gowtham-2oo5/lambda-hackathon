...
export async function GET(request: NextRequest) {
  try {
    ...
    const logGroups = logGroup && logGroup !== 'all' ? [logGroup] : defaultLogGroups;
    const allLogs: any[] = [];
    ...
    for (const group of logGroups) {
      try {
        const command = new FilterLogEventsCommand({
          logGroupName: group,
          startTime: startTime ? parseInt(startTime) : Date.now() - 24 * 60 * 60 * 1000, // Last 24 hours
        ...
        allLogs.push({
          ...
          level: 'WARN',
          service: extractServiceName(group),
          message: `Unable to fetch logs from ${group}: ${error instanceof Error ? error.message : 'Unknown error'}`,
